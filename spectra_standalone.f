...
!	It is for 1,2,and 3 D
     ...
     &	    ict(-150:150,-150:150,-150:150),! &  150 = maximum Nc
     &	    icum,ISPLINE,i1,j1,i2,j2,l1,l2,ic1,ic2,ic3,ic4,m,p, 
     &	    ic5,ic6,ic7,ic8
 ...
 404    read(40,1000) linemc
        if(index(linemc(1:72),'ed,V,tprime').ne.0 ) then
 ...
 2004     read(40,1000) linemc	
	do ic=1,Nc 					     				      	       					          
            if(index(linemc(1:72),'sigma_loc').ne.0 ) then  	      
	      do i=-nwn,nwn
                read(40,*,IOSTAT=istat) m,m,wn(i),r2,r3
                if(istat.ne.0) goto 997				      
...
!	Now that we have inputed all the data, allocate the internal arrays as to be the same dimension as input
	allocate(c1_sigma(1:nkpoints,-nf:nf),c_Gl1(1:nkpoints,-nf:nf),c_Gl2(1:nkpoints,-nf:nf),c_Gext(1:nkpoints,-nf:nf),
     &			c2_sigma(1:nkpoints,-nf:nf),c_Gl(1:nkpoints,-nf:nf))
...
!	Prepare the G_local
!###################################################
!	Note, I can easily combine the G_loc into one. Just for clarity I separate them
	G_loc=zeroc
	do n =-nwn,-nwn_c
	do ic=1,Nc
...
<<<<<<< spectra_standalone.f
		  	  c_Gl(ick,i)=blint(G_loc,k,0,0,i)
		      enddo	
=======
		  	  c_Gl(ick,i)=blint(G_loc,k,0,0,i)
		            c2_sigma(ick,-1-i)=c2_sigma(ick,i) !PBC
		  	   c_Gl(ick,-1-i)=c_Gl(ick,i)
		      enddo	
>>>>>>> 1.24
...
<<<<<<< spectra_standalone.f
		  	   c_Gl(ick,i)=blint(G_loc,kx,ky,kz,i)
=======
		  	   c_Gl(ick,i)=blint(G_loc,kx,ky,kz,i)
		            c2_sigma(ick,-1-i)=c2_sigma(ick,i) !PBC
		  	   c_Gl(ick,-1-i)=c_Gl(ick,i)
...
!	Note, there is no spline interpolation for 3D.
     ...
     &	    nkpoints, info, infot, ikx, iky, deg, itunnel,i_energy, 
     &	    ict(-150:150,-150:150,-150:150),! &  150 = maximum Nc
     &	    icum,ISPLINE,i1,j1,i2,j2,l1,l2,ic1,ic2,ic3,ic4, 
     &	    ic5,ic6,ic7,ic8
